...
class Calendar{
    ...
    public function DrawCalendar($month,$year){
        ...
        for($ListDay = 1; $ListDay <= $DaysInMonth; $ListDay++){
            ...
            $calendar.= '<a class="day-number" href="main/datesearch?day='.$ListDay.
                '&month='.$month.'&year='.$year.'">'.$ListDay.'</a>';
    ...
    /**
     *Функція формування навігації календаря
     *
     * @param $month
     * @param $year
     * @return array
     */
    public function CalendarControls($month,$year)
    {
        $CalNav['current_date'] = "".$month."-".$year."";
        $CalNav['next'] = '<a href="?month='.($month != 12 ? $month + 1 : 1).'&year='.
            ($month != 12 ? $year : $year + 1).'" class="control">Наступний місяць >></a>';

        $CalNav['previous'] = '<a href="?month='.($month != 1 ? $month - 1 : 12).'&year='.
            ($month != 1 ? $year : $year - 1).'" class="control"><< Попередній місяць</a> ';
